...
`property var`. Typed properties enable `qmlcachegen` AOT compilation to C++,
...
`qmlcachegen` AOT compilation. qmllint warns via the `unqualified` category.
...
`qmlcachegen` to resolve bindings statically. Data must be passed via
...
force interpreter fallback and prevent `qmlcachegen` AOT compilation.
